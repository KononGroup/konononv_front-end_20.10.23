...
let usersMore2000 = [];
const phoneNumbersUsersMore2000 = users.filter((user) => {
  
  let tmp = +user["balance"].replace(/[^0-9.]/g, '');
    if (tmp > 2000) {
      usersMore2000.push(user["phone"]);
    }
  
    return  usersMore2000
}) 
console.log(usersMore2000);




let sum = 0;
const usersBalanceSum = users.map((user) => {
  let tmp = +user["balance"].replace(/[^0-9.]/g, '');
  console.log(tmp);
  sum += tmp;
  return sum
